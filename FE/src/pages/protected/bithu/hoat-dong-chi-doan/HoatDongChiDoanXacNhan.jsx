...
const HoatDongChiDoanXacNhan = () => {
  ...
  const chiDoanActivityIds = useMemo(
    () =>
      MOCK_HOAT_DONG.filter((hd) => hd.donViToChuc !== "Đoàn Trường").map(
        (hd) => hd.idHD,
      ),
    [],
  );
      ...
      <div className="bt-header">
        <div>
          ...
          <p className="bt-subtitle">
            Xác nhận đoàn viên đã tham gia hoàn thành
          </p>
        </div>
            ...
            <tbody>
              {approved.map((reg, idx) => (
                <tr key={`${reg.idDV}-${reg.idHD}-${idx}`}>
                  ...
                  <td>
                    <span className="bt-status bt-status--approved">
                      Đã duyệt
                    </span>
                  </td>
                  <td>
              ...
              {approved.length === 0 && (
                <tr>
                  <td colSpan="6" className="bt-empty">
                    Không có đoàn viên nào chờ xác nhận
                  </td>
                </tr>
              )}
            </tbody>
