...
const HoatDongXacNhan = () => {
  ...
  const fetchActivities = useCallback(async () => {
    setLoading(true);
    try {
      const result = await hoatdongAPI.getAllSchoolActivities({
        page: 1,
        limit: 100,
      });
      if (result.success) {
  ...
  useEffect(() => {
    ...
    const fetch_ = async () => {
      ...
      try {
        const result = await doanviendangkiAPI.getApprovedActivityRegistrations(
          selectedHD.idHD,
        );
        if (result.success) {
  ...
  const handleXacNhan = async (hd) => {
    if (
      !window.confirm(
        `Xác nhận hoàn thành "${hd.tenHD}" và cộng +${hd.diemHD} điểm cho đoàn viên đã duyệt?`,
      )
    )
      return;
        ...
        <table className="hd-table">
          ...
          <tbody>
            {loading ? (
              ...
              filtered.map((hd) => (
                <tr key={hd.idHD}>
                  <td style={{ fontWeight: 700, color: "#0d1f3c" }}>
                    {hd.tenHD}
                  </td>
                  <td>
                  ...
                  <td style={{ color: "#004f9f", fontWeight: 800 }}>
                    +{hd.diemHD || 0}
                  </td>
                  <td>
            ...
            {!loading && filtered.length === 0 && (
              <tr>
                ...
                >
                  Không có hoạt động nào chờ xác nhận
                </td>
              </tr>
            )}
          </tbody>
