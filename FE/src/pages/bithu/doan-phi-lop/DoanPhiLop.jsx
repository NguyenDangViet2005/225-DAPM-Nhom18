...
const DoanPhiLop = () => {
  ...
  const handleTabChange = (tab) =>
    navigate(tab === 'gui' ? '/doan-phi-lop/gui' : '/doan-phi-lop/lap-danh-sach');
  ...
  return (
    <div className="bt-page">
      <div className="bt-header">
        ...
        <span className="bt-badge"><Wallet size={13} /> Chi đoàn: {MY_CHI_DOAN}</span>
      </div>

      {/* Stats */}
      <div className="bt-stats">
        {[
          { icon: Wallet,       label: `Mức phí ${currentRate.namHoc}`, value: `${currentRate.soTien.toLocaleString()} ₫`, color: '#004f9f' },
          { icon: CheckCircle,  label: 'Đã đóng',   value: `${daDong}/${chiDoanMembers.length}`, color: '#10b981' },
          { icon: Clock,        label: 'Chưa đóng', value: `${chuaDong}`,                        color: '#ef4444' },
          { icon: Wallet,       label: 'Tổng đã thu', value: `${tongThu.toLocaleString()} ₫`,    color: '#0369a1' },
        ].map((s, i) => (
          <div key={i} className="bt-glass bt-stat-card">
            ...
            <div>
              ...
              <p className="bt-stat-card__value" style={{ fontSize: '1.3rem' }}>{s.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bt-tabs">
        {[{ key: 'danh-sach', label: 'Lập danh sách thu' }, { key: 'gui', label: 'Gửi danh sách nộp' }].map(t => (
          ...
          <div className="bt-glass" style={{ padding: '0.875rem 1rem' }}>
            <div className="bt-toolbar">
              <div className="bt-search-wrap">
                <Search size={15} />
                <input className="bt-search-input" placeholder="Tìm MSSV hoặc tên đoàn viên..."
          ...
          <div className="bt-glass bt-table-card">
            <div style={{ overflowX: 'auto' }}>
              <table className="bt-table">
                <thead>
                  <tr><th>Đánh dấu</th><th>MSSV</th><th>Họ và Tên</th><th>Số tiền</th><th>Ngày đóng</th><th>Trạng thái</th></tr>
                </thead>
                <tbody>
                  {filtered.map(p => (
                    <tr key={p.idDoanPhi}>
                      <td><input type="checkbox" checked={!!checked[p.idDV]} onChange={() => toggleCheck(p.idDV)} disabled={p.trangThai === 'Đã đóng'} /></td>
                      <td style={{ fontWeight: 700, color: 'var(--bt-blue)' }}>{p.idDV}</td>
                      ...
                      <td><span className={`bt-status ${p.trangThai === 'Đã đóng' ? 'bt-status--paid' : 'bt-status--unpaid'}`}>{p.trangThai}</span></td>
                    </tr>
                  ))}
                  {filtered.length === 0 && <tr><td colSpan="6" className="bt-empty">Không tìm thấy đoàn viên nào</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
