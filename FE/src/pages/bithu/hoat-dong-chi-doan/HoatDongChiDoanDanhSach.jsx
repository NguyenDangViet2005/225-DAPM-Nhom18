...
const HoatDongChiDoanDanhSach = () => {
  ...
  const chiDoanActivities = useMemo(() => MOCK_HOAT_DONG.filter(hd => hd.donViToChuc !== 'Đoàn Trường'), []);
  ...
  const registrations = useMemo(() => {
    ...
    return MOCK_DANG_KY_HOAT_DONG.filter(reg => {
      ...
      const matchSearch = reg.hoTen.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const stats = {
    ...
    pending:  registrations.filter(r => r.trangThaiDuyet === 'Chờ duyệt').length,
    approved: registrations.filter(r => r.trangThaiDuyet === 'Đã duyệt').length,
    rejected: registrations.filter(r => r.trangThaiDuyet === 'Từ chối').length,
  };
    ...
    <div className="bt-page">
      <div className="bt-header">
        <div>
          ...
          <p className="bt-subtitle">Theo dõi đăng ký của đoàn viên trong chi đoàn</p>
        </div>
      </div>
      ...
      <div className="bt-glass" style={{ padding: '0.875rem 1rem' }}>
        <div className="bt-toolbar">
          <select className="bt-filter-select" value={selectedHD} onChange={e => setSelectedHD(e.target.value)}>
            ...
            {chiDoanActivities.map(hd => <option key={hd.idHD} value={hd.idHD}>{hd.tenHD}</option>)}
          </select>
          <div className="bt-search-wrap">
            <Search size={15} />
            <input className="bt-search-input" placeholder="Tìm MSSV, tên đoàn viên..."
      ...
      <div className="bt-glass bt-table-card">
        <div style={{ overflowX: 'auto' }}>
          <table className="bt-table">
            <thead>
              <tr><th>MSSV</th><th>Họ và Tên</th><th>Hoạt động</th><th>Ngày đăng ký</th><th>Trạng thái</th><th>Lý do từ chối</th></tr>
            </thead>
            <tbody>
              {registrations.map((reg, idx) => (
                <tr key={`${reg.idDV}-${reg.idHD}-${idx}`}>
                  ...
                  <td><span className={`bt-status ${STATUS_CLS[reg.trangThaiDuyet] || ''}`}>{reg.trangThaiDuyet}</span></td>
                  <td style={{ fontSize: '0.8rem', color: 'var(--bt-red)' }}>{reg.liDoTuChoi || '—'}</td>
                </tr>
              ))}
              {registrations.length === 0 && <tr><td colSpan="6" className="bt-empty">Không có đăng ký nào phù hợp</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
